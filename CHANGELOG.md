...
## [2026-08-09] - AWS Stolen Credential Permission Enumeration Chokepoint

First AWS entry in the repository. Existing cloud coverage was Microsoft identity only (Graph API recon, device-code phishing, PRT enrollment).

### Added

- `chokepoints/discovery/aws-credential-permission-enumeration.yml` — a discovery entry with an optional identity-orientation anchor followed by permission testing and capability confirmation, mapped to T1580 and T1526 only where List/Get/Describe calls actually discover services or infrastructure. The invariant is conditional empirical testing: a capability not already known from context or readable policy requires a relevant AWS API request and service authorization outcome. CloudTrail visibility is conditional on service and operation support, event-category selection, and whether the integration records denied requests. Identity-based and resource-based allows are generally unioned, permissions boundaries, Organizations SCPs/RCPs, and session policies constrain the result, and any explicit deny overrides. Four variations: Pacu, enumerate-iam, CloudFox, and manual AWS CLI.
- `sigma-rules/aws-credential-permission-enumeration/research.yml` — orientation and authorization-failure baseline for deriving local thresholds and building the chronic-denial allowlist.
- `sigma-rules/aws-credential-permission-enumeration/hunt.yml` — authorization-failure breadth across ≥5 distinct service namespaces, where those failures account for ≥0.5 of all events by the principal in the window. The distinct-namespace count discriminates better than raw volume, since a high denial count against a single API usually means misconfiguration.
- `sigma-rules/aws-credential-permission-enumeration/analyst.yml` — novel-denial breadth anti-joined against a 30-day baseline, correlated with a subsequent success in a just-denied namespace.

### Notes

- Cross-namespace breadth and the denied-then-allowed transition are labelled throughout as Hunt and Analyst heuristics for undirected enumeration rather than as unavoidable steps. Context, a readable policy, or a correct first guess can narrow the sweep to one call, and an operator can stop after the denials or succeed first.
- Aggregation logic (distinct-namespace counts, denial ratios, baseline anti-joins) cannot be expressed in standard Sigma. Each rule ships a partial stub plus one Athena implementation, following the stub-plus-query pattern established by `sigma-rules/graph-api-recon-burst/hunt.yml`. Athena is used rather than CloudWatch Logs Insights because a trail delivering to S3 is the common deployment; neither surface is automatic. Console event history covers 90 days without configuration, but the queries need an explicitly created trail delivering to S3 plus an Athena table over that data. Management events are selected by default when a trail is created; the S3 delivery and the table are not.
- The Analyst tier scores on novelty because rate thresholds are defeated by pacing, and its 30-day baseline holds every `(principal, eventSource, eventName)` tuple previously observed, successful or failed, so a routinely denied tuple does not read as novel every day. Novelty helps against a low call rate inside the observation window, but that window is one day, so multi-day pacing evades it unless the window is widened at query cost.
- Detections deliberately avoid `userAgent`. AWS SDKs and tools commonly expose application-ID or custom user-agent metadata, so tool-name matches such as `Pacu` or `Boto3` are not durable.
- The three tiers query CloudTrail management events for supported service operations. S3 object-level data events are optional coverage for reads such as `s3:GetObject` where a selector covers the bucket. Configuring coverage does not guarantee every denied request is recorded, so the entry keeps the AWS request and service outcome separate from conditional CloudTrail visibility.
- Baselines and aggregations key on `COALESCE(useridentity.sessioncontext.sessionissuer.arn, useridentity.arn, useridentity.principalid, useridentity.accesskeyid)`. A temporary `ASIA*` access key rotates per session, and an assumed-role ARN embeds a `RoleSessionName` that repeats only when the caller reuses it, so neither is a dependable long-term baseline identity. The Analyst correlation adds a tighter `actor_key` (access key first, then request ARN or principal ID, then the stable principal) so one session's denial under a shared role cannot join a different session's success. The Hunt and Analyst Athena CTEs also exclude `useridentity.type <> 'AWSService'` to match their Sigma service-principal filters, and environment-specific chronic and legitimate-principal allowlists must be applied to the Athena query by hand, since the Sigma placeholders have no effect on it.
- Only authorization and authentication failures count as denials — `AccessDenied`, `AccessDeniedException`, `UnauthorizedOperation`, `Client.UnauthorizedOperation`, `AuthorizationError`, `AuthFailure`. Throttling, validation, and missing-resource errors also populate `errorCode` and say nothing about the caller's permissions. Total event counts remain the ratio denominator, and the Sigma selections use the same list. The queries read the top-level `errorcode` column of the AWS sample CloudTrail table in Athena; AWS documents `errorCode` and `errorMessage` as optional record fields, and some services report authorization failures inside `responseElements` instead, so those services are a blind spot until the ingestion pipeline normalizes them into `errorcode`. `errorcode` is NULL on success in that table, so success is tested with `(errorcode IS NULL OR errorcode = '')`.
- No emulation script in this entry. An AWS emulation would need a disposable account with a deliberately under-permissioned key, so it is deferred to a follow-up.
- Thresholds (5 namespaces, 0.5 ratio, 3 novel namespaces, 30-day baseline) are starting points derived from the reasoning above rather than from measured telemetry. They should be re-derived per environment from the Research tier. No production-validation claim is made for this entry.
